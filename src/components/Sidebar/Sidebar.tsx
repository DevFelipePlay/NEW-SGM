    ...
    <Box>
      ...
      >
        ...
        >
          <Box>
            {isDrawerOpen ? (
              ...
              <AiOutlineMenuUnfold style={{ fontSize: '2rem', margin: '1rem' }} />
            )}
          </Box>
        </Box>
        {isDrawerOpen && (
        ...
        >
          {RowsSideBar.map((item, index) => (
            <Box
              ...
              key={index}
              onClick={() => handleIconClick(item.label, item.to, item.listItemsTabBar)}
            >
              {item.icon}
              {isDrawerOpen && <Typography sx={sidebarIconTextStyles}>{item.label}</Typography>}
            </Box>
          ))}
        </Box>
        ...
        >
          <Box
            sx={{
              ...sidebarIconStyles,
              ...(activeIcon === 'configurações' ? sidebarIconActiveStyles : {}),
              alignItems: 'center',
              justifyContent: 'initial',
            }}
          ...
          >
            <PiSignOut />
            {isDrawerOpen && <Typography sx={sidebarIconTextStyles}> Sair</Typography>}
          </Box>
        </Box>
      </Box>
      ...
      <Box
        ...
        sx={{
          ...
          boxShadow: 'rgba(0, 0, 0, 0.507) 0px 14px 28px, rgba(0, 0, 0, 0.22) 0px 10px 10px',
        }}
      >
        {activeIcon === 'Play Admin' && <Typography sx={titleTabBar}>Play Admin</Typography>}
        {activeIcon === 'Home' && <Typography sx={titleTabBar}>Home</Typography>}
        {activeIcon === 'Minha operadora' && (
          <Typography sx={titleTabBar}>Minha operadora</Typography>
        )}
        {activeIcon === 'Multi Nivel' && <Typography sx={titleTabBar}>Multi Nivel</Typography>}
        <Box sx={{ width: '100%' }}>
          {isTabBarOpen && (
            <Box
              sx={{
