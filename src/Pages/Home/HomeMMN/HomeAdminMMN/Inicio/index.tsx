...
const renderCustomizedLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent }: Iconst) => {
    ...
    <Grid container spacing={2} justifyContent='center'>
      ...
      >
        <Cards title={'Escala de usuários novos'} subTitle={''} size='90%'>
          <LineChart width={smDown ? 250 : mdDown ? 400 : 500} height={400} data={data}>
        ...
        <Cards title={'Relação entre ativações e recargas'} subTitle={''} size={'90%'}>
          <BarChart width={smDown ? 250 : mdDown ? 400 : 500} height={250} data={data}>
            <CartesianGrid strokeDasharray='3 3' />
            <XAxis dataKey='name' />
            <YAxis />
            <Tooltip />
        ...
        <Cards title={'Total de Usuários Ativos'} subTitle={'Usuários ativos'} size={'90%'}>
          <Typography variant={smDown ? 'h5' : 'h4'}>40.973 Pessoas Ativas</Typography>
        </Cards>
      </Grid>
      <Grid
        item
      ...
      >
        <Cards title={'Divisão do total de lucro'} subTitle={''} size={smDown ? '80%' : '100%'}>
        ...
        >
          ...
          <Box sx={{ mb: 1 }}>
            <Typography variant={smDown ? 'h5' : 'h4'}>Pacotes de chips</Typography>
