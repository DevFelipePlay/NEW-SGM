...
export function Faturas() {
  ...
  const [responseListAtivacaoERecarga, setResponseListAtivacaoERecarga] =
    useState<IResPostPlayVisualizaFaturas[]>([]);
  const [responseListPacotes, setResponseListPacotes] = useState<
    IResPostPlayVisualizaFaturasPacotes[]
  >([]);
  const { user } = useUser();
  const [currentPagePacotes, setCurrentPagePacotes] = useState(1);
  const [currentPageAtivacaoERecarga, setCurrentPageAtivacaoERecarga] =
    useState(1);
  ...
  const renderPaginatedListAtivacaoERecarga = () => {
    ...
    const paginatedItemsAtivacaoERecarga = responseListAtivacaoERecarga.slice(
      startIndex,
      endIndex
    );
    ...
    return paginatedItemsAtivacaoERecarga.map((item, index) => (
      <Box sx={{ width: "100%" }} key={index}>
        <ListFaturasAtivacaoERecarga
          ...
          dataDeCriacao={
            item.created ? dateFormatter(item.created) : "Sem data de criação"
          }
  ...
  const renderPaginatedListPacotes = () => {
    ...
    const endIndex = startIndex + itemsPerPage;
    const paginatedItemsPacotes = responseListPacotes.slice(
      startIndex,
      endIndex
    );
  ...
  return (
    <Box sx={{ width: "100%", typography: "body1", mt: 2 }}>
      <Typography
        ...
        variant="h6"
      >
        Escolha o tipo de Fatura que deseja visualizar
      </Typography>
                ...
                {loading ? (
                  ...
                  >
                    ...
                    <Stack spacing={2}>
                      <Pagination
                        count={Math.ceil(
                          responseListAtivacaoERecarga.length / itemsPerPage
                        )}
                ...
                {loading ? (
                  ...
                  >
                    ...
                    <Stack spacing={2}>
                      <Pagination
                        count={Math.ceil(
                          responseListPacotes.length / itemsPerPage
                        )}
